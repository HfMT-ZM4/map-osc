...
OSCAtom::OSCAtom(const OSCAtom & other) : type(other.type)
{
   switch (type) {
       ...
       case 's':
           ...
           break;
       case '.':
           map_val = std::make_unique<OSCMap>( *(other.map_val) );
           break;
       default:
...
T OSCAtom::get()
{
    ...
    else if constexpr ( std::is_same<T, OSCMap>::value )
    {
...
size_t OSCMap::getSerializedSizeInBytes() const
{
    ...
    for( auto& at : vec)
    {
        ...
        *ttptr++ = at->typetag();
        ...
        switch (at->typetag()) {
            ...
            case OSC_BUNDLE_TYPETAG:
            {
                const OSCMap& map = at->get<OSCMap>();
...
void OSCAtomVector::print(int tabs) const
{
    for( size_t i = 0; i < obj_vec.size(); i++ )
    {
        ...
        {
            printf("{\n");
            obj_vec[i]->get<OSCMap>().print(tabs+1);
